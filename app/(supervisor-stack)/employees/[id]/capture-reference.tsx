...
import {
  apiSupervisorCreateFaceEnrollments,
...
// Matching-shape fallback for when the native module didn't load (see the
// guarded require above) — lets the component call one hook unconditionally
// instead of branching on `useImageFaceDetector` at the call site. The
// choice between the two happens here, once, at module scope; by the time
// the component renders there is only ever one fixed function to call, so
// react-hooks/rules-of-hooks sees a plain unconditional hook call rather
// than a conditional one.
function useNoFaceDetector(_options: typeof FACE_DETECTOR_OPTIONS) {
  return null;
}
const useDeviceFaceDetector = useImageFaceDetector ?? useNoFaceDetector;
...
export default function CaptureReferencePhotosScreen() {
  ...
  // Returns null in Expo Go (see the guarded require above) or if the dev
  // client hasn't been rebuilt with the native module yet — useDeviceFaceDetector
  // resolves to useNoFaceDetector in that case (see its definition above).
  const faceDetector = useDeviceFaceDetector(FACE_DETECTOR_OPTIONS);
  ...
  const runUpload = useCallback(
    async (allPhotos: CapturedPhoto[]) => {
      ...
      try {
        ...
        const { results } = await apiSupervisorCreateFaceEnrollments(id, payload, {
          device,
        });
        setUploadProgress(1);
      ...
      >
        ...
        {step === "done" && (
          <View style={styles.stageScreen}>
            ...
            >
              <Text style={[typography.caption, { textAlign: "center" }]}>
                An admin will review and approve these photos before they&apos;re
