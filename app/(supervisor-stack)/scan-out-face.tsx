...
import {
  apiSupervisorScanOutConfirm,
  apiSupervisorScanOutIdentify,
  apiSupervisorScanOutPending,
  type SupervisorScanOutIdentifyResult,
} from "@/lib/apiClient";
import {
...
if (canUseFaceDetector) {
  try {
    ...
    console.warn("[supervisor scan-out-face] face detector module unavailable:", e);
  }
}
...
type TeamRow = {
  ...
  /** Auto-detected site/foreman for this employee's open scan — needed to
   * both display where they were clocked out from and to submit the right
   * siteId if this row later needs a manual confirm. */
  siteId?: string | null;
...
/**
 * Supervisor version of the foreman "clocking machine" continuous scanner
 * ((foreman-stack)/scan-out-face.tsx) — same camera-stays-on, workers-walk-
 * up loop, but with no site/foreman to pick first. The candidate pool is
 * every open scan-in across every site this supervisor is assigned to, so a
 * match auto-detects which site (and which foreman's crew) the employee
 * scanned in under, shown on the result card.
 */
export default function SupervisorScanOutFaceScanner() {
  ...
  const [result, setResult] = useState<SupervisorScanOutIdentifyResult | null>(null);
  ...
  const device = `${Device.modelName ?? Platform.OS} · Supervisor Clock Out Scanner`.slice(
    0,
    200,
  );
  ...
  const refreshCounts = useCallback(async () => {
    try {
      const res = await apiSupervisorScanOutPending(todayISO());
    ...
    (
      ...
      setTeamMembers((prev) => {
        ...
        next.set(id, {
          ...
          foremanName: foreman?.name ?? existing?.foremanName ?? null,
          matchedEnrollmentId:
  ...
  function showResult(
    next: Phase,
    res: SupervisorScanOutIdentifyResult | null,
    autoResume: boolean,
  ) {
    setResult(res);
  ...
  async function runPollTick() {
    ...
    try {
      ...
      const res = await apiSupervisorScanOutIdentify({
        dateISO: todayISO(),
        device,
        image: photo.base64,
        checkLiveness: false,
      ...
      if (!res.ok) {
        ...
        if (res.error === "no_candidates") {
          setPausedMessage(
            "Everyone across your sites has been scanned out, or no one has scanned in yet today.",
          );
      ...
      if (res.recorded) {
        markTeamMatch(res.employee, res.confidence, "clockedOut", res.site, res.foreman);
      ...
      console.error("[supervisor scan-out-face] poll tick failed:", e);
  ...
  const handleSubmitReviews = useCallback(async () => {
    ...
    try {
      ...
      for (const row of selectedReviewRows) {
        ...
        try {
          const confirmRes = await apiSupervisorScanOutConfirm({
            siteId: row.siteId,
            dateISO: todayISO(),
            employeeId: row.id,
            device,
          ...
          if (confirmRes.ok) {
            markTeamMatch(
              ...
              row.foremanName ? { id: "", name: row.foremanName } : null,
          ...
          console.error("[supervisor scan-out-face] submit review failed:", e);
          failed += 1;
        }
      }
      refreshCounts();
  ...
  if (phase === "permission") {
    return (
      <FaceScreenBackground>
        <Header title="Face Scan-Out" subtitle="All your sites" />
        <View
          style={[
            styles.body,
            styles.centerFill,
  ...
  return (
    <GlassPanel contentPadding={22} radius={14}>
      <View style={{ alignItems: "center", gap: spacing.sm }}>
        ...
        >
          Start the scanner when a team is ready to clock out. Workers simply
          walk up and look at the camera — their site and foreman are
    ...
    <GlassPanel contentPadding={22} radius={24}>
      <View style={{ alignItems: "center", gap: spacing.sm }}>
        ...
        >
          ...
          {totalToday === 1 ? "" : "s"} scanned in today across your sites
          have been clocked out. Nothing left for the scanner to do.
        </Text>
      </View>
    </GlassPanel>
  );
    ...
    >
      ...
      >
        <View>
          ...
          <Text style={typography.caption}>Live clock-out status, all sites</Text>
        </View>
        ...
        rows.map((row) => {
          ...
          const siteLine =
            row.siteName || row.foremanName
              ? [row.siteName, row.foremanName].filter(Boolean).join(" • ")
              : null;
...
function ResultCard({
  ...
  phase: Phase;
  result: SupervisorScanOutIdentifyResult;
  ...
  const { employee, site, foreman } = result;
  ...
  const siteLine = [site?.name, foreman?.name].filter(Boolean).join(" • ");
      ...
      <View style={{ alignItems: "center", gap: spacing.xxs }}>
        ...
        {!!siteLine && (
          ...
          >
            ...
            <Text style={typography.caption}>Auto-detected: {siteLine}</Text>
          </View>
        )}
